 ...
 *
 * POST → bot submits a move_execute intent, gets back a requestId
 * GET  → browser polls for pending requests to sign
 */
  ...
  try {
    ...
    if (!moduleAddress || !moduleName || !functionName) {
      return NextResponse.json(
        { error: "moduleAddress, moduleName, and functionName are required." },
        { status: 400 }
      );
    }
