...
The Planner uses the Initia MCP server as an on-chain oracle BEFORE
...
class OnChainVerification(BaseModel):
    needs_mcp_query: bool = Field(
        description=(
            ...
            "Examples: resolving a .init domain, checking a pool address exists, "
            "fetching token decimal precision."
        )
    )
    mcp_tool_name: Optional[str] = Field(
        default=None,
        description="MCP tool to call. Currently always 'move_view'.",
    )
    mcp_payload: Optional[Dict[str, Any]] = Field(
        default=None,
        description=(
            "Exact payload to POST to the MCP gateway. "
            "Must include: network, address, module, function, type_args, args."
        ),
    )
...
class PlannerState(BaseModel):
    ...
    collected_parameters: Dict[str, str] = Field(
        ...
        description=(
            ...
            "Keys match bot .env variable names (e.g. 'INITIA_POOL_A_ADDRESS')."
        ),
    )
...
PLANNER_SYSTEM = """\
You are the Planner Agent for Agentia, an Initia-native DeFi bot platform.
...
- yield_sweeper:      USER_WALLET_ADDRESS, INITIA_BRIDGE_ADDRESS, INITIA_USDC_METADATA_ADDRESS
- arbitrage:          INITIA_POOL_A_ADDRESS, INITIA_POOL_B_ADDRESS, INITIA_SWAP_ROUTER_ADDRESS,
                      INITIA_USDC_METADATA_ADDRESS, INITIA_EXECUTION_AMOUNT_USDC
- cross_chain_*:      INITIA_POOL_A_ADDRESS, INITIA_POOL_B_ADDRESS, INITIA_BRIDGE_ADDRESS,
                      USER_WALLET_ADDRESS, INITIA_USDC_METADATA_ADDRESS
- sentiment:          INITIA_POOL_A_ADDRESS, INITIA_POOL_B_ADDRESS, USER_WALLET_ADDRESS
- custom_utility:     only what the user explicitly provided

MCP VERIFICATION RULES:
1. If any address ends in ".init", set needs_mcp_query=true and build a move_view payload:
   address="0x1", module="initia_names", function="resolve", type_args=[], args=["<name>.init"]
2. If a pool address is provided, verify it with:
   address="0x1", module="dex", function="get_pool_info", type_args=[], args=["<pool_address>"]
3. If token decimals are unknown, query:
   address="<token_address>", module="fungible_asset", function="decimals",
   type_args=["0x1::fungible_asset::Metadata"], args=["<token_address>"]
...
class InitiaMCPClient:
    """Thin synchronous HTTP client for the local Initia MCP server."""

    def __init__(self, base_url: str = INITIA_MCP_BASE, timeout: float = MCP_TIMEOUT) -> None:
    ...
    def move_view(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        POST /initia/move_view — synchronous, raises on HTTP error.
        Returns the parsed JSON response dict.
        """
        url = f"{self.base_url}/initia/move_view"
        try:
        ...
        if isinstance(content, list):
            for item in content:
                ...
                try:
                    ...
                    text = str(text).strip()
                    if text.startswith(("init1", "0x")):
                        return text
        # Direct data field
        data = result.get("data")
        if isinstance(data, str) and data.strip().startswith(("init1", "0x")):
            return data.strip()
    return None
