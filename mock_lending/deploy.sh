...
for NODE in "${NODES[@]}"; do
  for ATTEMPT in 1 2 3; do
    echo "Deploying MockLending via ${NODE} (attempt ${ATTEMPT}/3)"
    if initiad move deploy \
      ...
      --chain-id "$CHAIN_ID" \
      --output json \
      --yes | tee /tmp/mock_lending_deploy_output.json; then
      DEPLOY_OK=1
      break 2
    fi
    sleep 2
...
echo "MockLending deployment submitted. Inspect /tmp/mock_lending_deploy_output.json"
