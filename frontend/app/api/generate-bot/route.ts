...
function buildSafeInitiaSpreadScannerIndexTs(): string {
  return [
    ...
    '  if (!ARBITRAGE_ROUTER_ARGS_TEMPLATE) return [buyEndpointAddress, sellEndpointAddress, EXECUTION_AMOUNT_USDC.toString()];',
    ...
    '  });',
    '  if (result) {',
    '    log("INFO", "[SUCCESS] Arbitrage transaction submitted");',
    '    return;',
    '  }',
...
function sanitizeInitiaIndexTemplates(files: GeneratedFile[], intent: Record<string, unknown>): GeneratedFile[] {
  if (String(intent.chain ?? "").toLowerCase() !== "initia") {
    return files;
  }

  const useYieldTemplate = isInitiaYieldSweeperIntent(intent);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    files = sanitizeInitiaIndexTemplates(files, intent);
