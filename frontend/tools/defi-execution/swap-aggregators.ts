...
export async function jupiterGetQuote(req: SwapQuoteRequest): Promise<SwapQuoteResponse> {
  ...
  return {
    ...
    routePlan: (data.routePlan ?? []).map((leg: Record<string, unknown>) => ({
...
export async function oneInchGetQuote(
  ...
  const protocols: RouteLeg[] = (data.protocols ?? [[]]).flat(2).map((p: Record<string, unknown>) => ({
  ...
  return {
    ...
    outputMint:           req.outputMint,
    inAmount:             req.amount,
    outAmount:            BigInt(data.dstAmount ?? "0"),
...
export async function zeroXGetQuote(
  ...
  return {
    inputMint:            req.inputMint,
    outputMint:           req.outputMint,
    inAmount:             BigInt(data.sellAmount ?? req.amount),
    outAmount:            BigInt(data.buyAmount ?? "0"),
    priceImpactPct:       parseFloat(data.estimatedPriceImpact ?? "0"),
    otherAmountThreshold: 0n,
    swapMode:             "ExactIn",
    routePlan:            [], // detailed routing available in /quote endpoint
    fees:                 data.fees ? [{ amount: BigInt(data.fees.integratorFee?.amount ?? "0"), mint: req.outputMint }] : [],
  };
}
